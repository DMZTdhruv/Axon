...
const WorkspaceTopBar = ({ currentWorkspace, folders }: { currentWorkspace: IUserWorkspace; folders: IRoutes[] }) => {
        ...
        <div className="flex gap-2 items-center">
          ...
          <div className="text-[13px] items-center gap-[5px] flex">
            {folders.map((folder, index) => {
              return (
                <Link href={`/workspace/${folder.workspace}/${folder._id}`} key={folder._id}>
                  {folder._id === currentWorkspace?._id ? (
                    <span className="hover:underline  shadow_topBar--text ">{folder.title}</span>
                  ) : (
                    <span className="text-neutral-300  hover:underline">{folder.title}</span>
                  )}
            ...
            <div className="bg-white rounded-[2.5px]  shadow_topBar--box flex items-center justify-center h-[15px] w-[15px] text-black text-[11px] leading-tight">
              <span className="font-bold">{currentWorkspace.workspace === "main" ? "M" : "E "}</span>
            </div>
          </div>
        </div>
        <BsThreeDots
