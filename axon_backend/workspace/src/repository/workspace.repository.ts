...
export class WorkspaceRepository {
	...
	async deleteWorkspaceContentByWorkspaceId(workspaceId: string) {
		try {
			const content = await WorkspaceContent.findOne({ workspaceId });
			if (!content) return;
			await sanityRepo.deleteImageById(content.imageId);
			await WorkspaceContent.deleteOne({ workspaceId });
		} catch (error) {
			if (error instanceof Error) {
				console.log(
					`Error from the deleteWorkspaceContentByWorkspaceId: ${error.message}`,
				);
			}
		}
	}
