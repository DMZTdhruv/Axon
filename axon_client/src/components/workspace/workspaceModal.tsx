				...
				<div>
					<div className="flex flex-col animate-in fade-in-0 gap-[0.40rem]">
						...
						<button
							...
							onClick={() => {
								addNewSubWorkspaceById(currentWorkspace._id, userId, workspaceType);
							}}
						>
						...
						>
							<LuArrowLeftRight className="text-neutral-400 group-active:scale-90 " size={18} />
							<span className="text-sm text-neutral-300">Move to</span>
							{moveToHover && <WorkspaceMoveToModal userId={userId} workspaceType={workspaceType} workspaceId={workspaceId} setMoveToHover={setMoveToHover} />}
						</div>
						...
						<button className="flex group w-full items-center gap-3 py-1 px-2 hover:bg-neutral-800/50 rounded-md cursor-pointer transition-colors" type="button" onClick={() => setModal(() => false)}>
...
const WorkspaceMoveToModal = ({ setMoveToHover, workspaceId, userId, workspaceType }: TWorkspaceMoveToModalProps) => {
	const workspaceStore = useWorkspaceStore();

	return (
		<div className="absolute group rounded-xl right-[-80%] top-0" onMouseLeave={() => setMoveToHover(() => false)}>
			<div className="relative p-[0.40rem] flex flex-col gap-[0.40rem] rounded-xl w-[250px]  border-neutral-800 border bg-neutral-950/50 parent_backdrop--blur">
				...
				{workspaceStore.workspace?.main?.map((workspaceLink) => {
					if (workspaceLink._id !== workspaceId) {
						return <WorkspaceModalMoveToSectionFolder currentWorkspaceId={workspaceId} workspaceLink={workspaceLink} key={workspaceLink.title} userId={userId} currentWorkspaceType={workspaceType} />;
					}
				})}
				<p className="text-[13px] text-neutral-600 pl-3">axonverse</p>
				{workspaceStore.workspace?.axonverse?.map((workspaceLink) => {
					if (workspaceLink._id !== workspaceId) {
						return <WorkspaceModalMoveToSectionFolder currentWorkspaceId={workspaceId} userId={userId} workspaceLink={workspaceLink} key={workspaceLink.title} currentWorkspaceType={workspaceType} />;
					}
				})}
			</div>
		</div>
