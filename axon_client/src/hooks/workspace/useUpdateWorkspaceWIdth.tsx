...
const updateWorkspaceWidth = async ({ workspaceId, newWidth, oldWidth, workspaceType, updateWorkspaceWidth }: UpdateWorkspaceWidthParams) => {
   const prevWidth = oldWidth;
   try {
      const response = await axios.post<CommonWorkspaceResponse>("http://localhost:3001/api/workspace/width/transaction", {
         workspaceId, width: newWidth
      }, {
