...
const MainNavigation = () => {
  ...
  const mainWorkspaceList: TNavigationWorkspaceContent[] | undefined = workspaceStore.workspace?.main?.map(
    (workspace) => {
      return {
        _id: workspace._id,
  ...
  const axonverseWorkspaceList: TNavigationWorkspaceContent[] | undefined = workspaceStore.workspace?.axonverse?.map(
    (workspace) => {
      return {
        _id: workspace._id,
        title: workspace.title,
  ...
  return (
    <div className="px-[40px] py-[20px] space-y-[20px]">
      <h2 className="font-bold text-[18px] text_glow ">Navigate easily to any page here</h2>
      <div className="flex gap-[20px] flex-col">
        <div className="flex flex-col gap-[10px]">
          {workspaceStore.workspace.main && <p className="text-[#595959] text-[13px]">Main</p>}
        ...
        <div className="flex flex-col gap-[10px]">
          {workspaceStore.workspace.axonverse && <p className="text-[#595959] text-[13px]">Axonverse</p>}
          <div className="flex gap-[10px] -translate-x-[5px]">
              ...
              {workspaceStore.workspace.recent.map((recentWorkspace) => {
                return (
                  ...
                  >
                    ...
                    />
                    <span className="text-[15px]  leading-tight">{recentWorkspace.title}</span>
                  </Link>
                );
              })}
            </div>
...
const NavItems = ({ _id, title, cover, icon, workspaceType }: TNavigationWorkspaceContent) => {
  const workspaceStore = useWorkspaceStore();
