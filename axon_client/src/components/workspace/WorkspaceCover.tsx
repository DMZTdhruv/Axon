...
const WorkspaceCover = ({ currentWorkspace }: { currentWorkspace: IUserWorkspace }) => {
  ...
  const handleImageChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      workspace.updateWorkspaceCover(currentWorkspace._id, currentWorkspace.workspace, imageUrl);
    }
  };
  ...
  return (
    <div className={`${currentWorkspace.cover ? "h-[290px]" : "h-[200px]"} group transition-all relative w-full `}>
      <div className=" fade-in-0 animate-in relative w-full h-full">
        {currentWorkspace.cover && (
          <div className="bg-gradient-to-b h-full w-full  select-none  from-slate-50/0 absolute z-[5] top-0 left-0 to-neutral-950/90 " />
        )}
        ...
        {currentWorkspace.cover && (
          <Image
            ...
            height={250}
            width={250}
            className={"w-full transition-all relative z-[1] h-full object-cover"}
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="text-[13px]  leading-tight opacity-65">Welcome to </span>
            <h1 className="text-[30px] leading-tight font-bold">{currentWorkspaceTitle}</h1>
          </div>
        </div>
      </div>
    </div>
